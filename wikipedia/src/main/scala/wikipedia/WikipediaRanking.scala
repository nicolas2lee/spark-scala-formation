...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd
    .filter(x => x.mentionsLanguage(lang)).count().toInt
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.mapValues(_.size).sortBy(_._2, ascending = false).collect().toList
  ...
  def printResult(result: List[(String, Int)]) = {
